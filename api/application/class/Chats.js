...
class Chats {
    ...
    newSocket(idSocket, data) {
        ...
        if (this.usersOnline[data.id] == undefined ) {
    ...
    logout(idSocket, idUser) {
        ...
        console.log('\n Usuarios en linea : ', Object.keys(this.usersOnline).length)
    ...
    async resetCounter(idUser, idChat){
        let chat = await this.getInbox( { idChat:idChat } );
        if( chat.f_id_usuario_a == idUser ){
            this.updateConter( idChat , 'a', 0 )
        }else{
            this.updateConter( idChat , 'b', 0 )
        }
    }
    ...
    notification( idUser , type , data ) {
 
        if (this.usersOnline[idUser]) {
            for (let socket in this.usersOnline[idUser].sockets) {
                console.log('socket', socket);
                this.io.to(socket).emit( 'Notify' ,  { type : type , data:data } );
            }
        } else {
        }
    }
