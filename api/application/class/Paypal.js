...
class Paypal {
	...
	async purchaseLyf(transactions, origin) {
		...
		return new Promise((resolve, reject) => {
			const payment_json = {
				...
				"redirect_urls": {
					"return_url": origin + "/Compras/Status",
					"cancel_url": origin + "/Compras/Status"
				},
	...
	async execute(paymentId, execute_payment_json) {
		return new Promise((resolve, reject) => {
			ppapi.payment.execute(paymentId, execute_payment_json, function (error, payment) {
				if (error) {
					console.log(error.response);
					reject(error);
				} else {
					// console.log('Pago desde paypal', JSON.stringify(payment));
					resolve(payment);
				}
			});
		});
