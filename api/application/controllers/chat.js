module.exports = (socket, io, chat) => {
	...
	socket.on('Message-Info', async (data, callback) => {

		callback(await model.InfoChat(data.idChat, socket.decoded_token.data.id));
	...
	socket.on('ResetCounter', (idChat, callback) => {
		...
		chat.resetCounter(socket.decoded_token.data.id, idChat);
		callback(true);
